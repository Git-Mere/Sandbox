...
void LockstepConnectingState_Init()
{
	...
	if ((connecting_socket == INVALID_SOCKET) &&
		LockstepConnectingState_HandleSocketError("Error creating lockstep connecting socket: "))
	{
		return;
	}

	//TODONE: make the socket non-blocking
	u_long mode = 1;
	ioctlsocket(connecting_socket, FIONBIO, &mode);

	//TODONE: set the UDP socket to reference the specified port on the local machine (127.0.0.1)
	...
	if (inet_pton(AF_INET, "127.0.0.1", &addr.sin_addr) <= 0 &&
		LockstepConnectingState_HandleSocketError("Error setting IP address of connecting socket: "))
	{
		return;
	}

	if (connect(connecting_socket, (sockaddr*)&addr, sizeof(addr)) == -1 &&
		LockstepConnectingState_HandleSocketError("Error connecting connecting socket: "))
	{
		return;
	}


	//TODONE: send a buffer containing the word "Lockstep" to the server
	char buffer[] = "Lockstep";
	if (send(connecting_socket, buffer, sizeof(buffer), 0) == -1 &&
		LockstepConnectingState_HandleSocketError("Error sending lockstep connecting socket: ")) 
	{
		return;
	}
...
void LockstepConnectingState_Update()
{
	...
	//TODONE: attempt to receive a response from a hosting server
	char buffer[4096];
	int res = recv(connecting_socket, buffer, sizeof(buffer), 0); // replace "0" with the recv call...
	if ((res == -1) &&
		LockstepConnectingState_HandleSocketError("Error receiving lockstep connecting socket: "))
	{
		return;
	}
