...
void LockstepGame::Update()
{
	...
	if (!is_local_update_sent_&& local_player_.has_uncommitted_move)
	{
		...
		//TODONE: send the state to the remote player
		// -- don't forget to handle errors appropriately with the LockstepGame_HandleSocketErrors function!
		if (send(socket_, network_buffer_, send_size, 0) == -1 &&
			LockstepGame_HandleSocketError("Error sending lockstep socket in gaming: "))
		{
			return;
		}
	...
	//TODONE: attempt to receive data from the remote player
	const int res = recv(socket_, network_buffer_, kNetworkBufferSize, 0);; // replace "0" with the recv call...
	if ((res == -1) &&
		LockstepGame_HandleSocketError("Error receiving lockstep socket in gaming: "))
	{
		return;
	};
...
{
	//TODONE: serialize the turn number, identifying this move
	reinterpret_cast<int*>(network_buffer_)[0] = turn_number_;

	//TODONE: serialize the uncommitted board position of the local player
	reinterpret_cast<int*>(network_buffer_)[1] = local_player_.uncommitted_position.x;
	reinterpret_cast<int*>(network_buffer_)[2] = local_player_.uncommitted_position.y;
...
void LockstepGame::DeserializeRemoteState(const size_t bytes_received)
{
	...
	//TODONE: deserialize the turn number
	const auto remote_turn_number = reinterpret_cast<int*>(network_buffer_)[0]; // replace "0" with the deserialized data...
	...
	//TODONE: deserialize the uncommitted position of the remote player
	remote_player_.uncommitted_position = { reinterpret_cast<int*>(network_buffer_)[1],reinterpret_cast<int*>(network_buffer_)[2] }; // replace "{0,0}" with the deserialized data
