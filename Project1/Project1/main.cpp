...
private:
    ...
    void bubble_up(int index) {
        int parent_p = parent(index);

        if (std::get<0>(data[parent_p]) > std::get<0>(data[index])) {
            swap(data[index], data[parent_p]);
            bubble_up(parent_p);
        }
        else {
            return;
    ...
    {
        int size_v = size();
        int left_c = left(index);
        int right_c = right(index);
        int min = left_c;

        if (right_c < size_v && min < size_v && std::get<0>(data[min]) > std::get<0>(data[right_c])) {
            min = right_c;
        }

        if (min < size_v && std::get<0>(data[index]) > std::get<0>(data[min])) {
